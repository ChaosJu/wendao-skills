...
> 来源：本文件由 `../马斯克决策模式与行为分析-20260404.md` 完整整合而成（full_merge），并保留原 sidecar 索引壳的标准化摘要（见文末）。补充背景另见 `../Elon-Musk-思想体系调研-20260404.md` 已整合进 `01-writings.md`；综合调研辅助见 `../research.md`。
...
- 典型决策模式：押上个人资本、压缩反馈周期、垂直整合关键瓶颈、用极端目标迫使组织重新设计。
- 正案例：SpaceX 可复用火箭、Tesla 制造与供应链整合、Starlink 快速部署。
- 反案例：Twitter/X 裁员与内容治理、FSD 时间线过度承诺、DOGE 式政府效率叙事。

### 来源可信度

- 一手：公司时间线、公开财报/文件、法庭证词、访谈。
- 二手：传记和媒体分析用于补足动机与影响。
