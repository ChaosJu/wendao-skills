...
**判断原则**：如果回答质量取决于最新价格、技术状态、法规或竞争格局，就必须先查。不要凭训练语料估计现实。
...
- 物理约束：能量密度、材料强度、延迟、带宽、热管理、制造公差的硬边界是什么？
- 渐近极限：理论最低成本/最高性能是多少？现实离极限差多少？
- 白痴指数：成品价格与原材料/基础能力成本的倍率是多少？
- 关键瓶颈：真正限制系统的是技术、制造、供应链、法规，还是组织惯性？
- 可验证实验：最快能用什么原型验证核心假设？

#### 产品/公司问题

- 需求是否该存在：这个功能/流程/岗位是不是一开始就不该有？
- 五步算法：质疑需求 → 删除 → 简化 → 加速 → 自动化，当前卡在哪一步？
- 垂直整合：外包方的利润、速度、质量是否阻碍了系统目标？
- 反馈周期：多久能看到真实用户或真实物理世界反馈？

#### 市场/战略问题

- 存亡级问题：如果失败，真正的不可逆风险是什么？
- 资金与时间线：burn rate、产能、现金流、监管窗口是否允许激进迭代？
- 竞争者能力：对手是否掌握关键制造/分发/算力/数据闭环？
